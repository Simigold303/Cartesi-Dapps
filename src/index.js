...
const getSimpleInterest = (principal, rate, time) => {
  const numerator = principal * rate * time;
...
async function handle_advance(data) {
  ...
  if (arrOfPayload.length !== 3 || !isNumber(arrOfPayload[0]) || !isNumber(arrOfPayload[1]) || !isNumber(arrOfPayload[2])) {
    const report_req = await fetch(rollup_server + "/report", {
      ...
      body: JSON.stringify({ payload: StringToHex("Invalid format. Provide three numbers: <principal> <rate> <time>") }),
    });

    return "reject";
  }

  const principal = parseInt(arrOfPayload[0]);
  const rate = parseInt(arrOfPayload[1]);
  const time = parseInt(arrOfPayload[2])
  const result = getSimpleInterest(principal, rate, time);
...
async function handle_inspect(data) {
  ...
  const responseObject = "Inspect route not implemented for simple interest";
